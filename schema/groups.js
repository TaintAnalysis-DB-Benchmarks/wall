...
const {
  packData: packImageData
} = require('./images'); // For performance profiling.
...
const resolvers = {
  ...
  Mutation: {
    addGroup: async (parent, {
      ...
      const found = await models.image_groups.findOne({
        where: {
          user_id,
          label
        }
      ...
      return await models.image_groups.create({
        user_id,
        label,
        description
      });
    ...
    updateGroup: async (parent, {
      ...
      if (label) {
        const labeled = await models.image_groups.findOne({
          where: {
            user_id,
            label
          }
    ...
    deleteGroup: async (parent, {
      ...
      await models.image_group_relationship.destroy({
        where: {
          group_id: id
        }
      });
      return await models.image_groups.destroy({
        where: {
          id
        }
      });
    },
    addGroupImage: async (parent, {
      groupId,
      imageId
    }, req) => {
      const currentUser = await getUser(req);
      const foundGroup = await models.image_groups.findOne({
        where: {
          id: groupId,
          user_id: currentUser.id
        }
      ...
      const foundImage = await models.images.findOne({
        where: {
          id: imageId
        }
      });
      ...
      const foundRelationship = await models.image_group_relationship.findOne({
        where: {
      ...
      await models.image_group_relationship.create({
        image_id: parseInt(foundImage.id),
        group_id: parseInt(foundGroup.id)
      }); // TODO: get a better return value from this method

      return 1;
    },
    removeGroupImage: async (parent, {
      groupId,
      imageId
    }, req) => {
      const currentUser = await getUser(req);
      ...
      return await models.image_group_relationship.destroy({
        where: {
...
const packData = async (group, currentUser, req) => {
  ...
  const imagess_njm4 = await models.images.findAll({
    where: {
      id: relationships.map(data => data.image_id)
    }
  });
