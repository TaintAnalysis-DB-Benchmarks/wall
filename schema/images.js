...
const {
  getUser
} = require('../authenticate'); // For profiling.
...
const resolvers = {
  ...
  Mutation: {
    addImage: async (parent, {
      ...
      if (fs.existsSync(`./${uploadDir}/${filename}`)) {
        ...
        await groups.forEach(async groupId => {
          ...
          const group = await models.image_groups.findOne({
            where: {
              id: groupId,
              user_id
            }
          ...
          if (group) {
            await models.image_group_relationship.create({
              image_id: image.id,
              group_id: groupId
            });
          }
    ...
    updateImage: async (parent, {
      ...
      const found = await models.images.findOne({
        where: {
          id
        }
      });
    ...
    removeImageFromGroup: async (parent, {
      imageId,
      groupId
    }, req) => {
      const found = await models.images.findOne({
      ...
      if (!currentUser || currentUser.id !== found.user_id) {// allow uses to remove images uploaded from other uses to their own groups
        //throw new AuthenticationError("Not authorized to update image");
      }

      models.image_group_relationship.destroy({
        where: {
          image_id: imageId,
          group_id: groupId
        }
    ...
    addImageToGroup: async (parent, {
      ...
      if (!currentUser || currentUser.id !== found.user_id) {// allow uses to add images uploaded from other uses to their own groups
        //throw new AuthenticationError("Not authorized to update image");
      }

      const relationship = await models.image_group_relationship.findOne({
        where: {
          image_id: imageId,
          group_id: groupId
        }
    ...
    deleteImage: async (parent, {
      ...
      return await models.images.destroy({
        where: {
          id
        }
      });
...
const packData = async (data, currentUser, req) => {
  ...
  if (currentUser) {
    ...
    const image_groupss_cxq3 = await models.image_groups.findAll({
      where: {
        id: relationships.map(data => data.group_id)
      }
    });
    const groups = await Promise.all(relationships.map(async rel => {
      const group = image_groupss_cxq3.find(x => x.id === rel.group_id);
      return group;
    }));
